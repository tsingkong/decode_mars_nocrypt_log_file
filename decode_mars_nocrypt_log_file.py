...
def DecodeBuffer(_buffer, _offset, _outbuffer):
    ...
    if not ret[0]:
        ...
        if -1==fixpos: 
            ...
            str_out = "[F]decode_log_file.py decode error len=%d, result:%s \n"%(fixpos, ret[1])
            _outbuffer.extend(bytes(str_out, 'utf-8'))
            _offset += fixpos 
    ...
    if MAGIC_NO_COMPRESS_START==magic_start or MAGIC_COMPRESS_START==magic_start or MAGIC_COMPRESS_START1==magic_start:
        ...
        str_out = 'in DecodeBuffer _buffer[%d]:%d != MAGIC_NUM_START'%(_offset, magic_start)
        _outbuffer.extend(bytes(str_out, 'utf-8'))
        return -1
    ...
    if seq != 0 and seq != 1 and lastseq != 0 and seq != (lastseq+1):
        str_out = "[F]decode_log_file.py log seq:%d-%d is missing\n" %(lastseq+1, seq-1)
        _outbuffer.extend(bytes(str_out, 'utf-8'))
    ...
    try:
        ...
        if MAGIC_NO_COMPRESS_START1==_buffer[_offset] or MAGIC_COMPRESS_START2==_buffer[_offset] or MAGIC_SYNC_ZSTD_START==_buffer[_offset] or MAGIC_ASYNC_ZSTD_START==_buffer[_offset]:
            print("use wrong decode script:", _buffer[_offset])
            return -1
            ...
            tmpbuffer = decompressor.decompress(memoryview(tmpbuffer))
        ...
        traceback.print_exc()
        str_out = "[F]decode_log_file.py decompress err, " + str(e) + "\n"
        _outbuffer.extend(bytes(str_out, 'utf-8'))
